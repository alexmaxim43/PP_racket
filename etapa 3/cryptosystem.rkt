...
(define (key n)
  (define e (cadr(get-nth-quadruple n)))
  (define f (caddr(get-nth-quadruple n)))
  (let((a1 (a1 e f)) (b1 (b1 e f)) (c1 (c1 e f))
                      (a2 (a2 e f)) (b2 (b2 e f)) (c2 (c2 e f))
                      (a3 (a3 e f)) (b3 (b3 e f)) (c3 (c3 e f)))
...
(define (message->codes message)
  (map (lambda (x) (if (equal? x 32) (- x 32) (- x 96))) (map char->integer (string->list message))
  ))
...
(define (codes->message codes)
  (list->string (map integer->char (map (lambda (x) (if (equal? x 0) (+ x 32) (+ x 96))) codes)))
  )
...
(define (first-x-numbers-of-list x L l)
  (if(<= x 0)
        l
        (first-x-numbers-of-list (- x 1) (cdr L) (append l (list (car L))))
        )
  )


(first-x-numbers-of-list 3 '(1 2 3 4 5 6) null)
...
(define (extend-key key size)
  ...
  ( cond
    ((equal? len size) key)
    ((> size (* 2 len)) (extend-key (append key key) size))
    ((> size len) (append key (first-x-numbers-of-list (- size len) key null )))
    ((< size len) (first-x-numbers-of-list size key null ) )
       )
    )
  )
...
(define (math op)
  (lambda (m k)
    (let(( extended-key (extend-key k (length m))))
    (map (lambda (x) (modulo x 27)) (map op m extended-key)))
  )
   )
...
(define (e-d-message F)
  (lambda (m k)
   (codes->message (F (message->codes m) k))
    )
  )


(define encrypt-message
  (e-d-message encrypt-codes)
  )
(define decrypt-message
  (e-d-message decrypt-codes)
  )
