...
(define (func x)
  (stream (multiply T1 x) (multiply T2 x) (multiply T3 x)))
(define ppt-stream-in-tree-order
  (let loop ((Q (stream-cons '(3 4 5) empty-stream)))
    (stream-cons (stream-first Q) (loop (stream-append (stream-rest Q) (func (stream-first Q)))))))
...
(define (pairs G H)
  (let loop ((g G)
    ...
    (if (< counterG counterH)
        (stream-cons (cons (stream-first g) (stream-first h)) (loop (stream-rest g) h (+ counterG 1) counterH  ))

        (stream-cons (cons (stream-first g) (stream-first h)) (loop G (stream-rest h) 1 (+ counterH 1)  )) 
...
(define (verif pereche)
  (if (and (odd? (car pereche)) (odd? (cdr pereche)) )
      (if (and (equal? (gcd (car pereche) (cdr pereche)) 1) (< (car pereche) (cdr pereche)) )
          #t
          #f
          )
       #f
...
(define gh-pairs-stream
 (stream-filter verif (pairs naturals (stream-rest naturals))))
; TODO
...
(define (func2 pereche)
  (append (list (* (car pereche) (cdr pereche))) (list (quotient (- (* (cdr pereche) (cdr pereche)) (* (car pereche) (car pereche))) 2))
         (list (quotient (+ (* (car pereche) (car pereche)) (* (cdr pereche) (cdr pereche))) 2)))
  )

(define ppt-stream-in-pair-order
  (stream-map func2 gh-pairs-stream))
